const App = {
    setup() {
        ...
        const validateForm = function () {
            ...
            if (!state.firstName) {
                state.errors.firstName = 'Họ và đệm không được để trống.';
                isValid = false;
            }
            if (!state.lastName) {
                state.errors.lastName = 'Tên không được để trống.';
                isValid = false;
            }
            if (!state.email) {
            ...
            if (state.id === '') {
                ...
                if (!state.confirmPassword) {
                    state.errors.confirmPassword = 'Xác nhận mật khẩu không được để trống.';
                    isValid = false;
                }
                if (state.password && state.confirmPassword && state.password !== state.confirmPassword) {
                    state.errors.confirmPassword = 'Mật khẩu và xác nhận mật khẩu không khớp.';
                    isValid = false;
                }
            }
        ...
        const validateChangePasswordForm = function () {
            ...
            if (!state.newPassword) {
                state.errors.newPassword = 'Mật khẩu mới không được để trống.';
                isValid = false;
            } else if (state.newPassword.length < 6) {
                state.errors.newPassword = 'Mật khẩu mới phải từ 6 ký tự trở lên.';
                isValid = false;
            }
            ...
            handleSubmit: async function () {
                try {
                    ...
                    if (response.data.code === 200) {
                        ...
                        if (!state.deleteMode) {
                            state.mainTitle = state.id === '' ? 'Thêm người dùng' : 'Sửa người dùng';
            ...
            create: async (dataSource) => {
                mainGrid.obj = new ej.grids.Grid({
                    ...
                    columns: [
                        ...
                        { field: 'firstName', headerText: 'Họ và đệm', width: 150, minWidth: 150 },
                        { field: 'lastName', headerText: 'Tên', width: 150, minWidth: 150 },
                        { field: 'email', headerText: 'Email', width: 150, minWidth: 150 },
                        { field: 'emailConfirmed', headerText: 'Đã xác thực Email', textAlign: 'Center', width: 150, minWidth: 150, type: 'boolean', displayAsCheckBox: true },
                        { field: 'isBlocked', headerText: 'Bị khóa', textAlign: 'Center', width: 150, minWidth: 150, type: 'boolean', displayAsCheckBox: true },
                        { field: 'isDeleted', headerText: 'Đã xóa', textAlign: 'Center', width: 150, minWidth: 150, type: 'boolean', displayAsCheckBox: true },
                        { field: 'createdAt', headerText: 'Ngày tạo', width: 150, format: 'yyyy-MM-dd HH:mm' }
                    ],
                    toolbar: [
                        'ExcelExport', 'Search',
                        { type: 'Separator' },
                        ...
                        { type: 'Separator' },
                        { text: 'Đổi mật khẩu', tooltipText: 'Đổi mật khẩu', id: 'ChangePasswordCustom' },
                        { text: 'Vai trò', tooltipText: 'Gán vai trò', id: 'ChangeRoleCustom' },
                    ],
                    beforeDataBound: () => { },
                    dataBound: function () {
                    ...
                    toolbarClick: async (args) => {
                        ...
                        if (args.item.id === 'AddCustom') {
                            ...
                            state.mainTitle = 'Thêm người dùng';
                            resetFormState();
                            mainModal.obj.show();
                        }
                        ...
                        if (args.item.id === 'EditCustom') {
                            ...
                            if (mainGrid.obj.getSelectedRecords().length) {
                                ...
                                state.mainTitle = 'Sửa người dùng';
                        ...
                        if (args.item.id === 'DeleteCustom') {
                            ...
                            if (mainGrid.obj.getSelectedRecords().length) {
                                const selectedRecord = mainGrid.obj.getSelectedRecords()[0];
                                state.mainTitle = 'Xóa người dùng?';
                                state.id = selectedRecord.id ?? '';
                                state.firstName = selectedRecord.firstName ?? '';
                                state.lastName = selectedRecord.lastName ?? '';
                                state.email = selectedRecord.email ?? '';
                        ...
                        if (args.item.id === 'ChangePasswordCustom') {
                            if (mainGrid.obj.getSelectedRecords().length) {
                                const selectedRecord = mainGrid.obj.getSelectedRecords()[0];
                                state.changePasswordTitle = 'Đổi mật khẩu người dùng';
                        ...
                        if (args.item.id === 'ChangeRoleCustom') {
                            if (mainGrid.obj.getSelectedRecords().length) {
                                const selectedRecord = mainGrid.obj.getSelectedRecords()[0];
                                state.changeRoleTitle = 'Cập nhật vai trò người dùng';
                                state.userId = selectedRecord.id ?? '';
            ...
            create: async (dataSource) => {
                secondaryGrid.obj = new ej.grids.Grid({
                    ...
                    columns: [
                        ...
                        { field: 'roleName', headerText: 'Vai trò', allowEditing: false, width: 200, minWidth: 200 },
                        { field: 'accessGranted', headerText: 'Cho phép truy cập', textAlign: 'Center', width: 150, minWidth: 150, editType: 'booleanedit', displayAsCheckBox: true, type: 'boolean', allowEditing: true },
                    ],
                    toolbar: [
                        'ExcelExport',
                        { type: 'Separator' },
