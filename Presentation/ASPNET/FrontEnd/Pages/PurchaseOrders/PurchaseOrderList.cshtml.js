const App = {
    setup() {
        ...
        const validateForm = function () {
            ...
            if (!state.orderDate) {
                state.errors.orderDate = 'Ngày đặt hàng không được để trống.';
                isValid = false;
            }
            if (!state.vendorId) {
                state.errors.vendorId = 'Nhà cung cấp không được để trống.';
                isValid = false;
            }
            if (!state.taxId) {
                state.errors.taxId = 'Thuế suất không được để trống.';
                isValid = false;
            }
            if (!state.orderStatus) {
                state.errors.orderStatus = 'Trạng thái đơn hàng không được để trống.';
                isValid = false;
            }
            ...
            handleFormSubmit: async () => {
                ...
                try {
                    ...
                    if (response.data.code === 200) {
                        ...
                        if (!state.deleteMode) {
                            state.mainTitle = 'Sửa đơn nhập hàng';
            ...
            create: async (dataSource) => {
                mainGrid.obj = new ej.grids.Grid({
                    ...
                    columns: [
                        ...
                        { field: 'number', headerText: 'Mã đơn nhập', width: 150, minWidth: 150 },
                        { field: 'orderDate', headerText: 'Ngày đặt hàng', width: 150, format: 'yyyy-MM-dd' },
                        ...
                        { field: 'taxName', headerText: 'Thuế suất', width: 150, minWidth: 150 },
                        { field: 'afterTaxAmount', headerText: 'Tổng thanh toán', width: 150, minWidth: 150, format: 'N2' },
                        { field: 'createdAtUtc', headerText: 'Ngày tạo', width: 150, format: 'yyyy-MM-dd HH:mm' }
                    ],
                    toolbar: [
                        'ExcelExport', 'Search',
                        { type: 'Separator' },
                        { text: 'Thêm', tooltipText: 'Tạo đơn mới', prefixIcon: 'e-add', id: 'AddCustom' },
                        { text: 'Sửa', tooltipText: 'Sửa đơn nhập', prefixIcon: 'e-edit', id: 'EditCustom' },
                        { text: 'Xóa', tooltipText: 'Xóa đơn nhập', prefixIcon: 'e-delete', id: 'DeleteCustom' },
                        { type: 'Separator' },
                        { text: 'In đơn PDF', tooltipText: 'Xuất PDF', id: 'PrintPDFCustom' },
                    ],
                    beforeDataBound: () => { },
                    dataBound: function () {
                    ...
                    toolbarClick: async (args) => {
                        ...
                        if (args.item.id === 'AddCustom') {
                            ...
                            state.mainTitle = 'Tạo mới đơn nhập hàng';
                            resetFormState();
                            state.secondaryData = [];
                        ...
                        if (args.item.id === 'DeleteCustom') {
                            ...
                            if (mainGrid.obj.getSelectedRecords().length) {
                                const selectedRecord = mainGrid.obj.getSelectedRecords()[0];
                                state.mainTitle = 'Xóa đơn nhập hàng?';
                                state.id = selectedRecord.id ?? '';
                                state.number = selectedRecord.number ?? '';
                                state.orderDate = selectedRecord.orderDate ? new Date(selectedRecord.orderDate) : null;
                                state.description = selectedRecord.description ?? '';
