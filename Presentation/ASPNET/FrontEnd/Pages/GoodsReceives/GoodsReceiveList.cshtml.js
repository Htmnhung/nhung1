    ...
    setup() {
        ...
        const validateForm = function () {
            ...
            if (!state.receiveDate) {
                state.errors.receiveDate = 'Ngày nhập kho không được để trống.';
                isValid = false;
            }
            if (!state.purchaseOrderId) {
                state.errors.purchaseOrderId = 'Đơn nhập hàng không được để trống.';
                isValid = false;
            }
            if (!state.status) {
            ...
            handleSubmit: async function () {
                try {
                    ...
                    if (response.data.code === 200) {
                        ...
                        if (!state.deleteMode) {
                            state.mainTitle = 'Sửa phiếu nhập kho';
            ...
            create: async (dataSource) => {
                mainGrid.obj = new ej.grids.Grid({
                    ...
                    columns: [
                        ...
                        { field: 'number', headerText: 'Mã phiếu', width: 150, minWidth: 150 },
                        { field: 'receiveDate', headerText: 'Ngày nhập kho', width: 150, format: 'yyyy-MM-dd' },
                        { field: 'purchaseOrderNumber', headerText: 'Đơn nhập hàng', width: 150, minWidth: 150 },
                    ...
                    toolbar: [
                        ...
                        { type: 'Separator' },
                        { text: 'In PDF', tooltipText: 'In phiếu PDF', id: 'PrintPDFCustom' },
                    ],
                    beforeDataBound: () => { },
                    dataBound: function () {
                    ...
                    toolbarClick: async (args) => {
                        ...
                        if (args.item.id === 'AddCustom') {
                            ...
                            state.mainTitle = 'Thêm phiếu nhập kho';
                            resetFormState();
                        ...
                        if (args.item.id === 'DeleteCustom') {
                            ...
                            if (mainGrid.obj.getSelectedRecords().length) {
                                const selectedRecord = mainGrid.obj.getSelectedRecords()[0];
                                state.mainTitle = 'Xóa phiếu nhập kho?';
                                state.id = selectedRecord.id ?? '';
                                state.number = selectedRecord.number ?? '';
                                state.receiveDate = selectedRecord.receiveDate ? new Date(selectedRecord.receiveDate) : null;
                                state.description = selectedRecord.description ?? '';
