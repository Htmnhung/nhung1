    ...
    setup() {
        ...
        const validateForm = function () {
            ...
            if (!state.deliveryDate) {
                state.errors.deliveryDate = 'Ngày xuất kho không được để trống.';
                isValid = false;
            }
            if (!state.salesOrderId) {
                state.errors.salesOrderId = 'Đơn bán hàng không được để trống.';
                isValid = false;
            }
            if (!state.status) {
                state.errors.status = 'Trạng thái không được để trống.';
                isValid = false;
            }
            ...
            handleSubmit: async function () {
                try {
                    ...
                    if (response.data.code === 200) {
                        ...
                        if (!state.deleteMode) {
                            state.mainTitle = 'Sửa phiếu xuất kho';
            ...
            create: async (dataSource) => {
                mainGrid.obj = new ej.grids.Grid({
                    ...
                    columns: [
                        ...
                        { field: 'number', headerText: 'Số phiếu xuất', width: 150, minWidth: 150 },
                        { field: 'deliveryDate', headerText: 'Ngày xuất kho', width: 150, format: 'yyyy-MM-dd' },
                        { field: 'salesOrderNumber', headerText: 'Đơn bán hàng', width: 150, minWidth: 150 },
                    ...
                    toolbar: [
                        ...
                        { type: 'Separator' },
                        { text: 'In phiếu xuất PDF', tooltipText: 'Xuất PDF', id: 'PrintPDFCustom' },
                    ],
                    beforeDataBound: () => { },
                    dataBound: function () {
                    ...
                    toolbarClick: async (args) => {
                        ...
                        if (args.item.id === 'AddCustom') {
                            ...
                            state.mainTitle = 'Thêm phiếu xuất kho';
                            resetFormState();
                        ...
                        if (args.item.id === 'DeleteCustom') {
                            ...
                            if (mainGrid.obj.getSelectedRecords().length) {
                                const selectedRecord = mainGrid.obj.getSelectedRecords()[0];
                                state.mainTitle = 'Xóa phiếu xuất kho?';
                                state.id = selectedRecord.id ?? '';
                                state.number = selectedRecord.number ?? '';
                                state.deliveryDate = selectedRecord.deliveryDate ? new Date(selectedRecord.deliveryDate) : null;
                                state.description = selectedRecord.description ?? '';
