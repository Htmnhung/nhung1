const App = {
    setup() {
        const state = Vue.reactive({
            ...
            newPassword: '',
            confirmNewPassword: '',
            mainTitle: 'Sửa hồ sơ cá nhân',
            changePasswordTitle: 'Đổi mật khẩu',
            changeAvatarTitle: 'Thay ảnh đại diện',
            errors: {
                firstName: '',
                lastName: '',
                oldPassword: '',
            ...
            create: async (dataSource) => {
                mainGrid.obj = new ej.grids.Grid({
                    ...
                    columns: [
                        ...
                        { field: 'firstName', headerText: 'Họ và đệm', width: 200, minWidth: 200 },
                        { field: 'lastName', headerText: 'Tên', width: 200, minWidth: 200 },
                        { field: 'companyName', headerText: 'Cửa hàng / Công ty', width: 400, minWidth: 400 },
                    ],
                    toolbar: [
                        'ExcelExport', 'Search',
                        { type: 'Separator' },
                        { text: 'Sửa', tooltipText: 'Sửa hồ sơ', prefixIcon: 'e-edit', id: 'EditCustom' },
                        { type: 'Separator' },
                        { text: 'Đổi mật khẩu', tooltipText: 'Đổi mật khẩu', id: 'ChangePasswordCustom' },
                        { text: 'Thay ảnh đại diện', tooltipText: 'Thay ảnh đại diện', id: 'ChangeAvatarCustom' },
                    ],
                    beforeDataBound: () => { },
                    dataBound: function () {
        ...
        const handler = {
            handleSubmit: async () => {
                ...
                // Validasi firstName
                if (!state.firstName) {
                    state.errors.firstName = 'Họ không được để trống.';
                    isValid = false;
                }
            ...
            handleChangePassword: async () => {
                ...
                if (!state.oldPassword) {
                    state.errors.oldPassword = 'Mật khẩu cũ không được để trống.';
                    isValid = false;
                } else if (state.oldPassword.length < 6) {
                    state.errors.oldPassword = 'Mật khẩu cũ phải từ 6 ký tự trở lên.';
                    isValid = false;
                }
                ...
                if (!state.confirmNewPassword) {
                    state.errors.confirmNewPassword = 'Xác nhận mật khẩu mới không được để trống.';
                    isValid = false;
                } else if (state.confirmNewPassword.length < 6) {
                    state.errors.confirmNewPassword = 'Mật khẩu xác nhận phải từ 6 ký tự trở lên.';
                    isValid = false;
                }
                ...
                try {
                    ...
                    if (response.status === 200) {
                        ...
                        Swal.fire({
                            ...
                            text: "Đã cập nhật ảnh đại diện mới!",
                            timer: 1000,
                            showConfirmButton: false
                        });
                        ...
                        Swal.fire({
                            ...
                            text: response.message ?? "Đã xảy ra lỗi trong quá trình tải lên."
                        });
                    }
                } catch (error) {
                    Swal.fire({
        ...
        const initDropzone = () => {
            if (!dropzoneInitialized && imageUploadRef.value) {
                ...
                const dropzoneInstance = new Dropzone(imageUploadRef.value, {
                    ...
                    acceptedFiles: "image/*",
                    addRemoveLinks: true,
                    dictDefaultMessage: "Kéo và thả hình ảnh vào đây để tải lên",
