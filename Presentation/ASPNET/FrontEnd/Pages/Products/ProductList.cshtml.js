const App = {
    setup() {
        ...
        const validateForm = function () {
            ...
            if (!state.name) {
                state.errors.name = 'Tên thiết bị không được để trống.';
                isValid = false;
            }
            if (!state.unitPrice) {
                state.errors.unitPrice = 'Đơn giá bán không được để trống.';
                isValid = false;
            } else if (!/^\d+(\.\d{1,2})?$/.test(state.unitPrice)) {
                state.errors.unitPrice = 'Đơn giá bán phải là số hợp lệ.';
                isValid = false;
            }
            if (!state.productGroupId) {
                state.errors.productGroupId = 'Hãng điện thoại không được để trống.';
                isValid = false;
            }
            if (!state.unitMeasureId) {
                state.errors.unitMeasureId = 'Đơn vị tính không được để trống.';
                isValid = false;
            }
            ...
            create: async (dataSource) => {
                mainGrid.obj = new ej.grids.Grid({
                    ...
                    columns: [
                        ...
                        { field: 'number', headerText: 'Mã thiết bị', width: 200, minWidth: 200 },
                        { field: 'name', headerText: 'Tên thiết bị', width: 200, minWidth: 200 },
                        { field: 'productGroupName', headerText: 'Hãng điện thoại', width: 150, minWidth: 150 },
                        { field: 'unitPrice', headerText: 'Đơn giá bán', width: 150, minWidth: 150, format: 'N2' },
                        { field: 'unitMeasureName', headerText: 'Đơn vị tính', width: 150, minWidth: 150 },
                        { field: 'physical', headerText: 'Sản phẩm vật lý', width: 200, minWidth: 200, textAlign: 'Center', type: 'boolean', displayAsCheckBox: true },
